...
pub struct BlockSumReduce4 {
    shader_source: String,
    workgroup_size: u32,
}

impl BlockSumReduce4 {
    pub fn new() -> Self {
        let workgroup_size = DEVICE_LIMITS.get().unwrap().max_compute_workgroup_size_x;
        let mut shader_source = include_str!("../shaders/block_sum_reduce_4.wgsl").to_string();
...
impl Default for BlockSumReduce4 {
    fn default() -> Self {
        Self::new()
    }
}

impl Kernel1D for BlockSumReduce4 {
    fn name(&self) -> &str {
        "block_sum_reduce_4"
    }
